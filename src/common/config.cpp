...
// GR2launcher defaults: fullscreen "Fullscreen" mode + Fifo present mode.
static ConfigEntry<bool> isFullscreen(true);
static ConfigEntry<string> fullscreenMode("Fullscreen");
static ConfigEntry<string> presentMode("Fifo");
static ConfigEntry<bool> isHDRAllowed(false);
// GR2launcher defaults: FSR/RCAS both off by default.
...
// gr2fork: GR2-specific runtime override knobs
...
// GR2launcher default: pipeline (shader) cache on by default to avoid recompile stutter.
static ConfigEntry<bool> pipelineCacheEnable(true);
static ConfigEntry<bool> pipelineCacheArchive(false);
// gr2fork: Vulkan descriptor-update path overrides
...
static ConfigEntry<bool> showFpsCounter(false); // gr2fork
static ConfigEntry<bool> logEnabled(true);

// gr2fork: non-toml runtime state
...
// ---- gr2fork: Vulkan descriptor-update overrides ----------------------------
...
void load(const std::filesystem::path& path, bool is_game_specific) {
    ...
    if (data.contains("Debug")) {
        ...
        showFpsCounter.setFromToml(debug, "showFpsCounter", is_game_specific); // gr2fork
...
void save(const std::filesystem::path& path, bool is_game_specific) {
    ...
    if (!is_game_specific) {
        ...
        data["Debug"]["showFpsCounter"] = showFpsCounter.base_value; // gr2fork
    }
...
void setDefaultValues(bool is_game_specific) {
    ...
    // GR2launcher mandated defaults — applied for BOTH a fresh config.toml
    // (via the ConfigEntry initializers above) and "Restore Defaults" (here).
    // These were previously gated on is_game_specific=true so global Restore
    // Defaults didn't touch them; that's no longer the case.
    isDevKit.set(false, is_game_specific);
    isNeo.set(false, is_game_specific); // isPS4Pro
    ...
    pipelineCacheEnable.set(true, is_game_specific);
    ...
    // GR2launcher defaults: fullscreen on with "Fullscreen" mode, Fifo present mode.
    isFullscreen.set(true, is_game_specific);
    fullscreenMode.set("Fullscreen", is_game_specific);
    presentMode.set("Fifo", is_game_specific);
    isHDRAllowed.set(false, is_game_specific);
    // GR2launcher defaults: FSR/RCAS both off.
    fsrEnabled.set(false, is_game_specific);
    rcasEnabled.set(false, is_game_specific);
    ...
    showFpsCounter.set(false, is_game_specific); // gr2fork
