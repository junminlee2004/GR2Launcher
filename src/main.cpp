...
int main(int argc, char* argv[]) {
         ...
         [&](int&) {
             std::cout
                 << "Usage: GR2launcher [options]\n"
                    "Options:\n"
                    "  No arguments: Opens the GUI. The launcher auto-detects an emulator\n"
                    "                executable beginning with 'shadPS4gr2fork' next to itself.\n"
                    "  -e, --emulator <path>         Explicit path to the emulator executable\n"
                    "                                (overrides auto-detect). Any non-path\n"
                    "                                value falls back to auto-detect.\n"
                    "  -g, --game <ID|path>          Specify game to launch.\n"
                    "  -d                            Alias for '-e default' (auto-detect).\n"
    ...
    if (has_emulator_argument) {
        ...
        if (std::filesystem::exists(emulator)) {
            // Caller passed an explicit absolute/relative path — use it as-is.
            emulator_path = emulator;
        } else {
            // Any other token ("default", a name from the old version manager,
            // anything) → fall back to auto-detect. The version manager is gone.
            emulator_path = EmulatorPath::FindGr2forkExecutable();
        }
        if (!std::filesystem::exists(emulator_path)) {
            const auto err = QObject::tr(
                "exe not found: no file beginning with \"%1\" was found in the launcher's "
                "directory.")
                                 .arg(EmulatorPath::kGr2forkExecutablePrefix);
            std::cerr << err.toStdString() << "\n";
            if (show_gui) {
                QMessageBox::critical(nullptr, "GR2launcher", err);
            }
            return 1;
        }
        if (!show_gui) {
        ...
        // Headless path requires `-g GAME` plus the auto-detected emulator; pure
        // GUI launches resolve the emulator lazily at "Play" time (also via
        // auto-detect), so no early check is needed here.
        // However, we still warm-fail at GUI startup: if no exe is co-located,
        // pop an error and quit before the user sees an empty main window.
        if (!has_command_line_argument) {
            const auto emu = EmulatorPath::FindGr2forkExecutable();
            if (emu.empty() || !std::filesystem::exists(emu)) {
                const auto err = QObject::tr(
                    "exe not found: no file beginning with \"%1\" was found in the launcher's "
                    "directory.\n\nPlace the gr2fork emulator executable next to GR2launcher and "
                    "restart.")
                                     .arg(EmulatorPath::kGr2forkExecutablePrefix);
                QMessageBox::critical(nullptr, "GR2launcher", err);
                return 1;
