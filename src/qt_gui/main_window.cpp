...
void MainWindow::AddUiWidgets() {
    ...
    // Version manager removed — the emulator is auto-detected next to the
    // launcher (see EmulatorPath::FindGr2forkExecutable).
}
...
void MainWindow::StartEmulator(std::filesystem::path path, QStringList args) {
    ...
    // Auto-detect the gr2fork emulator executable co-located with the launcher.
    // No more version manager — the exe must live next to GR2launcher and its
    // filename must start with "shadPS4gr2fork".
    const auto emu_path = EmulatorPath::FindGr2forkExecutable();
    if (emu_path.empty()) {
        QMessageBox::critical(
            nullptr, "GR2launcher",
            tr("exe not found: no file beginning with \"%1\" was found in the launcher's "
               "directory.\n\nPlace the gr2fork emulator executable next to GR2launcher.")
                .arg(EmulatorPath::kGr2forkExecutablePrefix));
        return;
    }

    QFileInfo fileInfo(QString::fromStdWString(emu_path.wstring()));
    if (!fileInfo.exists()) {
