...
KBMSettings::KBMSettings(std::shared_ptr<GameInfoClass> game_info_get,
    ...
    // gr2fork: mouse_default_mode combobox. Item text is the user-visible
    // label; the actual value written to the config is its lowercase form
    // (joined with underscores). See save/load logic below.
    ui->MouseDefaultModeBox->addItem("Off");
    ui->MouseDefaultModeBox->addItem("Joystick");
    ui->MouseDefaultModeBox->addItem("Gyro");
    ui->MouseDefaultModeBox->addItem("Touchpad");
    ui->MouseDefaultModeBox->addItem("Touchpad Swipe");
...
void KBMSettings::SaveKBMConfig(bool close_on_save) {
    ...
    add_mapping(ui->MouseGyroRollModeButton->text(), "mouse_gyro_roll_mode");
    ...
    // gr2fork: write mouse_default_mode + touchpad_swipe_enabled. The
    // combobox stores user-visible labels ("Off", "Joystick", "Gyro",
    // "Touchpad", "Touchpad Swipe"); convert to the lowercase identifiers
    // gr2fork's input parser expects ("off", "joystick", "gyro", "touchpad",
    // "touchpad_swipe").
    {
        const QString label = ui->MouseDefaultModeBox->currentText();
        std::string mode_str;
        if (label == "Off")
            mode_str = "off";
            ...
            mode_str = "gyro";
        else if (label == "Touchpad")
            mode_str = "touchpad";
        else if (label == "Touchpad Swipe")
            mode_str = "touchpad_swipe";
        else
            mode_str = "off"; // sanity fallback
        lines.push_back("mouse_default_mode = " + mode_str);
    }
...
void KBMSettings::SetDefault() {
    ...
    // gr2fork additions: all unmapped by default. The user wires what they
    // want -- touchpad swipes are best bound to a held-pad-key chord, gyro
    // roll mode is typically a held modifier (rctrl, mb_side, etc).
    ...
    ui->MouseGyroRollModeButton->setText("unmapped");

    ui->MouseJoystickBox->setCurrentText("none");
    // gr2fork: sensible defaults are "Off" + swipe disabled. The user opts
    // into the gr2fork-specific behaviour explicitly.
    ui->MouseDefaultModeBox->setCurrentText("Off");
    ...
    while (std::getline(file, line)) {
        ...
        if (!controllerInputdetected) {
            if (output_string == "cross") {
                ...
                ui->MouseGyroRollModeButton->setText(QString::fromStdString(input_string));
                ...
                // gr2fork. Convert lowercase identifier back to display label.
                QString label = "Off";
                if (input_string == "joystick")
                    ...
                    label = "Touchpad Swipe";
                else if (input_string == "off" || input_string == "none")
                    label = "Off";
                // Unknown values silently fall back to "Off" so a malformed
                // line doesn't crash the dialog.
                ui->MouseDefaultModeBox->setCurrentText(label);
            } else if (output_string == "touchpad_swipe_enabled") {
                // gr2fork. Accept "true"/"1" as true; anything else is false.
