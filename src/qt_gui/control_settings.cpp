...
void ControlSettings::SetDefault() {
    ...
    // gr2fork additions: unmapped by default. Users bind these when they
    // want the touchpad-direction outputs or swipe gestures.
...
void ControlSettings::SetUIValuestoMappings() {
    ...
    bool CrossExists = false, CircleExists = false, SquareExists = false, TriangleExists = false,
         ...
         // gr2fork additions
         TouchpadUpExists = false, TouchpadDownExists = false,
         TouchpadSwipeUpExists = false, TouchpadSwipeDownExists = false,
         TouchpadSwipeLeftExists = false, TouchpadSwipeRightExists = false,
         MouseGyroRollModeExists = false;
    int lineCount = 0;
    std::string line = "";
    while (std::getline(file, line)) {
        lineCount++;
    ...
    // gr2fork additions: default to unmapped if not present in the file.
    if (!TouchpadUpExists)
        ui->TouchpadUpButton->setText("unmapped");
    if (!TouchpadDownExists)
        ui->TouchpadDownButton->setText("unmapped");
    if (!TouchpadSwipeUpExists)
        ui->TouchpadSwipeUpButton->setText("unmapped");
    if (!TouchpadSwipeDownExists)
        ui->TouchpadSwipeDownButton->setText("unmapped");
    if (!TouchpadSwipeLeftExists)
        ui->TouchpadSwipeLeftButton->setText("unmapped");
    if (!TouchpadSwipeRightExists)
        ui->TouchpadSwipeRightButton->setText("unmapped");
    if (!MouseGyroRollModeExists)
        ui->MouseGyroRollModeButton->setText("unmapped");
