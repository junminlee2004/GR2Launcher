...
SettingsDialog::SettingsDialog(std::shared_ptr<gui_settings> gui_settings,
    ...
    // gr2fork: Force/Disable push descriptors are mutually exclusive — checking
    // one auto-clears the other so the user can't end up with both true.
    connect(ui->forcePushDescriptorsCheckBox, &QCheckBox::toggled, this, [this](bool checked) {
        if (checked && ui->disablePushDescriptorsCheckBox->isChecked()) {
            ui->disablePushDescriptorsCheckBox->setChecked(false);
        }
    });
    connect(ui->disablePushDescriptorsCheckBox, &QCheckBox::toggled, this, [this](bool checked) {
        if (checked && ui->forcePushDescriptorsCheckBox->isChecked()) {
            ui->forcePushDescriptorsCheckBox->setChecked(false);
        }
    });
    ...
    // Descriptions
    {
        ...
        // gr2fork
        ui->GR2GroupBox->installEventFilter(this);
        ui->aspectRatioComboBox->installEventFilter(this);
        ui->aspectRatioLabel->installEventFilter(this);
        ui->resolutionOverrideComboBox->installEventFilter(this);
        ui->resolutionOverrideLabel->installEventFilter(this);
        ui->disableMotionBlurCheckBox->installEventFilter(this);
        ...
        ui->forcePushDescriptorsCheckBox->installEventFilter(this);
        ui->disablePushDescriptorsCheckBox->installEventFilter(this);
        ...
        // gr2fork
        ui->loadAutoPatchesCheckBox->installEventFilter(this);
    }
...
void SettingsDialog::LoadValuesFromConfig() {
    ...
    ui->RCASSlider->setValue(toml::find_or<int>(data, "GPU", "rcasAttenuation", 250));
    ui->RCASValue->setText(QString::number(ui->RCASSlider->value() / 1000.0, 'f', 3));
    // gr2fork: GR2 patches group
    {
        const std::string aspect =
            toml::find_or<std::string>(data, "GPU", "aspectRatioOverride", "16:9");
        const int aspectIdx = ui->aspectRatioComboBox->findText(QString::fromStdString(aspect));
        ui->aspectRatioComboBox->setCurrentIndex(aspectIdx >= 0 ? aspectIdx : 0);

        const std::string resolution =
            toml::find_or<std::string>(data, "GPU", "resolutionOverride", "Off");
        // Accept aliases "4K"->"2160p", "8K"->"4320p"
        QString resQ = QString::fromStdString(resolution);
        if (resQ.compare("4K", Qt::CaseInsensitive) == 0)
            resQ = "2160p";
        else if (resQ.compare("8K", Qt::CaseInsensitive) == 0)
            resQ = "4320p";
        const int resIdx = ui->resolutionOverrideComboBox->findText(resQ);
        ui->resolutionOverrideComboBox->setCurrentIndex(resIdx >= 0 ? resIdx : 0);

        ui->disableMotionBlurCheckBox->setChecked(
            toml::find_or<bool>(data, "GPU", "disableMotionBlur", false));
    }
    ...
    // gr2fork
    ui->showFpsCounterCheckBox->setChecked(
        toml::find_or<bool>(data, "Debug", "showFpsCounter", false));
    ui->forcePushDescriptorsCheckBox->setChecked(
        toml::find_or<bool>(data, "Vulkan", "forcePushDescriptors", false));
    ui->disablePushDescriptorsCheckBox->setChecked(
        toml::find_or<bool>(data, "Vulkan", "disablePushDescriptors", false));
    ui->loadAutoPatchesCheckBox->setChecked(Config::getLoadAutoPatches());
...
void SettingsDialog::updateNoteTextEdit(const QString& elementName) {
    ...
    if (elementName == "consoleLanguageGroupBox") {
        ...
        text = tr("Update Compatibility On Startup:\\nAutomatically update the compatibility database when GR2launcher starts.");
    ...
    if (elementName == "hideCursorGroupBox") {
        ...
        text = tr("Enable Controller Background Input:\\nAllow GR2launcher to detect controller inputs when the game window is not in focus.");
    }
    ...
    } else if (elementName == "GR2GroupBox") {
        text = tr("GR2 Patches (gr2fork):\\nGR2-specific runtime patches applied to the game binary at load. These do not affect other games.\\nRequires a game restart to take effect.");
    } else if (elementName == "aspectRatioComboBox" || elementName == "aspectRatioLabel") {
        text = tr("Aspect Ratio Override:\\nRewrites the game's hardcoded 16:9 projection math at load time.\\n16:9 = native (Off). 16:10, 21:9, and 32:9 widen the field of view.\\nComposes with Resolution Override (final framebuffer = chosen density x chosen aspect).");
    } else if (elementName == "resolutionOverrideComboBox" || elementName == "resolutionOverrideLabel") {
        text = tr("Resolution Override:\\nRuntime-patches the 37 hardcoded 1920x1080 W/H/halfW/halfH/invW/invH constants in the game binary (plus the broader \"recommended\" set).\\nOff = native 1080p. 540p / 720p / 900p / 1080p / 1440p / 2160p (4K) / 4320p (8K). Composes with Aspect Ratio Override.");
    } else if (elementName == "disableMotionBlurCheckBox") {
        text = tr("Disable Motion Blur:\\nSkips the motion-blur post-process pass at game load by NOPing the two pass-registration calls in the render-graph builder.\\nIndependent of Resolution Override — can be toggled at native resolution.");
    }
    ...
    if (elementName == "gameFoldersGroupBox" || elementName == "gameFoldersListWidget") {
        ...
        text = tr("Portable user folder:\\nStores GR2launcher settings and data that will be applied only to the GR2launcher build located in the current folder. Restart the app after creating the portable user folder to begin using it.");
    ...
    if (elementName == "debugDump") {
        ...
        text = tr("Show FPS Counter:\\nOverlays the current FPS on the emulator window. The counter color reflects the rate (green at target, red when far below).");
    } else if (elementName == "forcePushDescriptorsCheckBox") {
        text = tr("Force Push Descriptors:\\nForces the Vulkan renderer to use the VK_KHR_push_descriptor path for descriptor updates, even if the driver advertises an alternative.\\nFor advanced debugging — leave off unless you know you need it.");
    } else if (elementName == "disablePushDescriptorsCheckBox") {
        text = tr("Disable Push Descriptors:\\nForces the Vulkan renderer to avoid VK_KHR_push_descriptor and fall back to descriptor sets.\\nFor advanced debugging — leave off unless you know you need it.\\nMutually exclusive with Force Push Descriptors.");
    ...
    } else if (elementName == "loadAutoPatchesCheckBox") {
        text = tr("Load Auto Patches (gr2fork):\\nOn game start, automatically apply the bundled patches for the running title (memory patches, shader patches, etc.).\\nDisable to run unmodified, e.g. for clean repro of upstream behavior.\\nThis is a runtime-only switch and is not persisted to the game's TOML.");
    }
...
void SettingsDialog::UpdateSettings(bool is_specific) {
    ...
    // gr2fork: GR2 patches + Vulkan descriptor overrides + FPS counter
    Config::setAspectRatioOverride(ui->aspectRatioComboBox->currentText().toStdString(),
                                   is_specific);
    Config::setResolutionOverride(ui->resolutionOverrideComboBox->currentText().toStdString(),
                                  is_specific);
    Config::setDisableMotionBlur(ui->disableMotionBlurCheckBox->isChecked(), is_specific);
    Config::setVkForcePushDescriptors(ui->forcePushDescriptorsCheckBox->isChecked(), is_specific);
    Config::setVkDisablePushDescriptors(ui->disablePushDescriptorsCheckBox->isChecked(),
                                        is_specific);
    Config::setShowFpsCounter(ui->showFpsCounterCheckBox->isChecked(), is_specific);
    ...
    if (!is_specific) {
        ...
        Config::setLoadAutoPatches(ui->loadAutoPatchesCheckBox->isChecked()); // gr2fork
